...
OpenFlowSwitchNetDevice::BufferFromPacket (Ptr<const Packet> constPacket, Address src, Address dst, int mtu, uint16_t protocol)
{
  ...
  /* MPLS support */
  /*
  bool mpls_set_it_back = false;
  MPLSHeader mpls(0);
  if( protocol == ETH_TYPE_MPLS_UNICAST ){
    NS_LOG_INFO ("The packet is MPLS labelled.");
    mpls_set_it_back = true;
    packet->RemoveHeader(mpls);
    protocol = Ipv4L3Protocol::PROT_NUMBER;
  }
  */
...
/*
  if (mpls_set_it_back){
    action_push_mpls apm;
    apm.label_out = htonl(mpls.GetLabel());
    sw_flow_key key;
    flow_extract(buffer,0,&key.flow);
    key.flow.dl_type = htons(0x0800);
    push_mpls_act (NULL, buffer ,&key , &apm);              // set MPLS tag
  }
*/
  return buffer;
}
                                            ...
                                            const Address& src, const Address& dst, PacketType packetType)
{
  ...
  NS_LOG_INFO ("Received packet from " << Mac48Address::ConvertFrom (src) << " looking for " << dst48 << " local is " << m_address);
              ...
              if (packetType == PACKET_OTHERHOST && dst48 == m_address)
              ...
              else
                {
                  if (packetType != PACKET_OTHERHOST)
                    {
                      NS_LOG_DEBUG ("It is not otherhost");
                      m_rxCallback (this, packet, protocol, src);
                    }
      ...
      if (p.netdev != 0 && !(p.config & OFPPC_PORT_DOWN))
        {
          ...
          if (p.netdev->SendFrom (data.packet->Copy (), data.src, data.dst, data.protocolNumber))
            {
              p.tx_packets++;
              p.tx_bytes += data.packet->GetSize();//bufsize;
            }
          else
            {
              p.tx_dropped++;
...
OpenFlowSwitchNetDevice::RunThroughFlowTable (uint32_t packet_uid, int port, bool send_to_controller)
{
  ...
  if (flow_extract (buffer, port != -1 ? port : OFPP_NONE, &key.flow) && (m_flags & OFPC_FRAG_MASK) == OFPC_FRAG_DROP)
    {
      NS_LOG_DEBUG ("matching key extraction fails");
      ofpbuf_delete (buffer);
      return;
    }
  ...
  if (port != -1)
    {
      ...
      if (config & (OFPPC_NO_RECV | OFPPC_NO_RECV_STP)
        ...
        {
          NS_LOG_DEBUG ("the port does not accept packets");
          return;
        }
    }
...
/*
bool
OpenFlowSwitchNetDevice::PushMPLSTag (uint32_t packet_uid, MPLSHeader &mpls){
  if( m_packetData.find (packet_uid)->second.protocolNumber == Ipv4L3Protocol::PROT_NUMBER ){ // IP
    m_packetData.find (packet_uid)->second.protocolNumber = ETH_TYPE_MPLS_UNICAST;

    m_packetData.find (packet_uid)->second.packet->AddHeader(mpls);

    return true; // success
  }
  return false;
} // OpenFlowSwitchNetDevice::PushMPLSTag

bool
OpenFlowSwitchNetDevice::PopMPLSTag(uint32_t packet_uid){
  MPLSHeader mpls(0);

  m_packetData.find (packet_uid)->second.packet->RemoveHeader(mpls);

  m_packetData.find (packet_uid)->second.protocolNumber = Ipv4L3Protocol::PROT_NUMBER;

  return true; // success
} // OpenFlowSwitchNetDevice::PopMPLSTag
*/
} // namespace ns3
