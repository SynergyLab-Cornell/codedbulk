...
TcpSocketBase::RecvFrom (uint32_t maxSize, uint32_t flags, Address &fromAddress)
{
  ...
  if (packet != nullptr && packet->GetSize () != 0)
    {
      //REVISED
      // a signal sent back to inform the upstream to send more data
      // which is not needed in practice
...
//REVISED
void
TcpSocketBase::SetMaxRto (Time maxRto)
{
  m_use_maxRto = true;
  m_maxRto = maxRto;
}
