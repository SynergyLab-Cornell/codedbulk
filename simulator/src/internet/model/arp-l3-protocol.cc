...
void
ArpL3Protocol::AddCacheEntry (Ptr<NetDevice> device, Ipv4Address dest_IP, Address dest_MAC) {
  NS_LOG_FUNCTION (this);
  Ptr<ArpCache> cache = FindCache (device);
  ArpCache::Entry *entry = cache->Lookup (dest_IP);
  if ( entry == 0 ) {
    // add the entry
    entry = cache->Add (dest_IP);
  }
  entry->SetMacAddress (dest_MAC);
  entry->MarkPermanent ();
}
