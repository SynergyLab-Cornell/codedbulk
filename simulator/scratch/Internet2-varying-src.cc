...
// testing coding
NS_LOG_COMPONENT_DEFINE ("Internet2BulkVarySrc");
...
main (int argc, char *argv[])
{
  ...
  CommandLine cmd;
  cmd.AddValue ("s", "number of sources", TOTAL_NUM_TRAFFIC);
  cmd.Parse (argc, argv);

  std::string sim_name = "Internet2-src";
  std::stringstream sim_serial;
  sim_serial << "-" << TOTAL_NUM_TRAFFIC;
