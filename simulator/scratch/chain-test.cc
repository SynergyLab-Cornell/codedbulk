...
#include "settings/topology/chain.cc_part"

// testing coding
NS_LOG_COMPONENT_DEFINE ("ChainTest");
...
#define USER_DEFINED_TRAFFIC() \
  ...
  for (int i = 1; i < 5; ++i) {\
    traffic->addDst(map_host_to_sw[i]);\
  }\
...
main (int argc, char *argv[])
{
  ...
  std::string sim_name = "Chain-Test";
  std::stringstream sim_serial;
  sim_serial.str("");

  // test
  CodedBulkCodec::setStoreAndForward (true);
