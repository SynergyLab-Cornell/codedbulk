...
CodedBulkGraph::DijkstraFromCodedBulkNode (int node_id) {
    for(std::vector<CodedBulkNode>::iterator it = _all_nodes.begin(); it != _all_nodes.end(); ++it){
        (*it)._dijkstra_metric    = std::numeric_limits<double>::max();
        (*it)._dijkstra_travelled = false;
        (*it)._dijkstra_upstream_edge_id = -1;
    }
    
    Ptr<CodedBulkNode> current_node   = &_all_nodes[node_id];
    if( !current_node->_marked )  return;    
    
    Ptr<CodedBulkNode> temp_node = nullptr;
