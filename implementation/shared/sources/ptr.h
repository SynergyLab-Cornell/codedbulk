...
public:
  ...
  T &operator * ();
  /**
   * Test for nullptr pointer.
   *
   * This enables simple nullptr pointer checks like
   * \code
   *   Ptr<..> p = ...;
   *   if (!p) ...
   * \endcode
   * \returns \c true if the underlying pointer is nullptr.
   */
  bool operator! ();
  /**
   * Test for non-nullptr pointer.
   *
